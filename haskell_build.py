# Copy from Default/exec.py to fix multiline matching problem.
...
class HaskellBuild(sublime_plugin.WindowCommand, ProcessListener):
    BLOCK_SIZE = 2**14
    ...
    def finish(self, proc):
        ...
        if self.show_errors_inline:
            # errs = self.output_view.find_all_results_with_text()
            errs = self.regex.findall(text)
            errs_by_file = {}
            for file, line, column, text in errs:
                line, column = map(int, (line, column))
                if file not in errs_by_file:
                    errs_by_file[file] = []
                errs_by_file[file].append((line, column, text))
            self.errs_by_file = errs_by_file
    ...
    def update_phantoms(self):
        stylesheet = '''
            <style>
                ...
                div.error span.message li {
                    margin: 0;
                    padding: 0;
                    padding-left: 0.7rem;
                }
                div.error span.message ul {
                    padding: 1rem;
                    margin: 0;
                }
                ...
                a {
                    ...
                    padding: 0.25rem 0.7rem 0.5rem 0.7rem;
                    font-weight: bold;
                    text-align: center;
                    line-height: 1.5;
                }
        ...
        for file, errs in self.errs_by_file.items():
            ...
            if view:
                ...
                for line, column, text in errs:
                    ...
                    if '•' in text:
                        text = re.sub(r'\n\ *•\ *', '</li><li>', text, flags=re.M)
                        text = re.sub(r'^\ *•\ *', '<li>', text, flags=re.M)
                        # text = re.sub(r'$', '</li>', text, flags=re.M)
                        text = re.sub(r'\n\ *', '<br>', text, flags=re.M)
                    else:
                        text = '<li>' + text.strip()
                    ...
                    phantoms.append(sublime.Phantom(
                        ...
                        ('<body id=inline-error>' + stylesheet +
                            ...
                            '<span class="message"><ul>' + text + '</li></ul></span>' +
                            '' +
                            '</div><a href=hide>CLOSE</a>' +
                            '</body>'),
